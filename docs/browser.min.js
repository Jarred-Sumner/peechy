var peechy = (() => {
  ...
  var _ByteBuffer = class {
    ...
    readUint32ByteArray() {
      ...
      return new Uint32Array(array.buffer, 0, array.length / Uint32Array.BYTES_PER_ELEMENT);
    }
    readInt8ByteArray() {
      const array = this.readByteArray();
      return new Int8Array(array.buffer, 0, array.length / Int8Array.BYTES_PER_ELEMENT);
    }
    readInt16ByteArray() {
      const array = this.readByteArray();
      return new Int16Array(array.buffer, 0, array.length / Int16Array.BYTES_PER_ELEMENT);
    }
    readInt32ByteArray() {
      const array = this.readByteArray();
      return new Int32Array(array.buffer, 0, array.length / Int32Array.BYTES_PER_ELEMENT);
    }
    readFloat32ByteArray() {
      const array = this.readByteArray();
      return new Float32Array(array.buffer, 0, array.length / Float32Array.BYTES_PER_ELEMENT);
    }
    readVarFloat() {
    ...
    _growBy(amount) {
      if (this.length + amount > this._data.length) {
        let data = new Uint8Array(Math.imul(this.length + amount, _ByteBuffer.WIGGLE_ROOM) << 1);
        data.set(this._data);
        this._data = data;
      }
      this.length += amount;
    ...
    writeUint16ByteArray(value) {
      this.writeByteArray(new Uint8Array(value.buffer, value.byteOffset, value.byteLength));
    }
  ...
  ByteBuffer.WIGGLE_ROOM = 1;
  ByteBuffer.LOW_PRECISION_VALUE = 10 ** 3;
  ...
  function parse(tokens) {
      ...
      if (definition.extensions) {
        for (let extension of definition.extensions) {
          ...
          if (otherDefinition.name !== extension || otherDefinition.kind !== "STRUCT") {
            error(`Expected ${otherDefinition.name} to to be a struct`, definition.line, definition.column);
          }
    ...
    for (let partName in picks) {
      ...
      for (let i = 0; i < fields.length; i++) {
        ...
        if (!foundMatch) {
          error(`Expected field ${name} to exist in ${definition.name}`, token.line, token.column);
        }
        fields[i] = {
          name: field.name,
          line: field.line,
      ...
      if (definition.kind === "UNION") {
        ...
        for (let j = 0; j < fields.length; j++) {
          ...
          if (state2[field.name]) {
            error("The type " + quote(field.type) + " can only appear in  " + quote(definition.name) + " once.", field.line, field.column);
          }
          state2[field.name] = 1;
          if (definedTypes.indexOf(field.type) === -1) {
            error("The type " + quote(field.type) + " is not defined for union " + quote(definition.name), field.line, field.column);
          }
        }
        ...
        if (!(definitions[field.name] || nativeTypeMap[field.name])) {
          error("Expected type used in alias to exist.", definition.line, definition.column);
        }
      } else {
        for (let j = 0; j < fields.length; j++) {
          let field = fields[j];
          ...
          if (field.type === "discriminator") {
            error("discriminator is only available inside of unions.", field.line, field.column);
          }
        }
      }
      let values = [];
    ...
    if (definition.kind === "UNION") {
      ...
      if (hasDiscriminator) {
        ...
        for (let i = 1; i < definition.fields.length; i++) {
          ...
          lines.push(`    case ${field.value}:`, indent + "var result = " + ("decode" + field.name) + "(bb);", indent + `result[${quote(definition.fields[0].name)}] = ${field.value};`, indent + `return result;`);
        }
      } else {
        ...
        for (let i = 0; i < definition.fields.length; i++) {
          let field = definition.fields[i];
          lines.push(`    case ${field.value}:`, indent + `return ${"decode" + field.name}(bb)`);
        }
      }
    } else {
      lines.push(`function ${functionName}(bb) {`);
      if (!withAllocator) {
        ...
        lines.push("  var result = Allocator[" + quote(definition.name) + "].alloc();");
      }
      lines.push("");
      ...
      for (let i = 0; i < definition.fields.length; i++) {
        ...
        switch (fieldType) {
          ...
          default: {
            ...
            if (!type) {
              error("Invalid type " + quote(fieldType) + " for field " + quote(field.name), field.line, field.column);
        ...
        if (field.isArray) {
          if (field.isDeprecated) {
            ...
            switch (fieldType) {
              ...
              case "uint16": {
                lines.push(indent + "result[" + quote(field.name) + "] = bb.readUint16ByteArray();");
                break;
              }
              case "uint32": {
                lines.push(indent + "result[" + quote(field.name) + "] = bb.readUint32ByteArray();");
                break;
              }
              case "int8": {
                lines.push(indent + "result[" + quote(field.name) + "] = bb.readInt8ByteArray();");
                break;
              }
              case "int16": {
                lines.push(indent + "result[" + quote(field.name) + "] = bb.readInt16ByteArray();");
                break;
              }
              case "int32": {
                lines.push(indent + "result[" + quote(field.name) + "] = bb.readInt32ByteArray();");
                break;
              }
              case "float32": {
                lines.push(indent + "result[" + quote(field.name) + "] = bb.readFloat32ByteArray();");
                break;
              }
              default: {
                lines.push(indent + "var length = bb.readVarUint();");
          ...
          lines.push(indent + "result[" + quote(field.name) + `] = ${"decode" + fieldType}(bb);`);
        } else if (fieldType && definitions[fieldType] && definitions[fieldType].kind === "UNION") {
          const key = quote(field.name + "Type");
          lines.push(indent + "result[" + key + "] = bb.readVarUint();", indent + "result[" + quote(field.name) + `] = ${"decode" + fieldType}(bb, result[${key}]);`);
        } else {
          if (field.isDeprecated) {
  ...
  function compileEncode(functionName, definition, definitions, aliases) {
    ...
    if (definition.kind === "UNION") {
      ...
      if (hasDiscriminator) {
        lines.push(`  type = type ? type : ${definition.name}[message[${quote(discriminator.name)}]];`);
        lines.push(`  if (!type) throw new Error('Expected message[${quote(discriminator.name)}] to be one of ' + JSON.stringify(${definition.name}) + ' ');`);
      } else {
        lines.push(`  if (!type) throw new Error('Expected type to be one of ' + JSON.stringify(${definition.name}, null, 2) + ' ');`);
      }
      lines.push("");
      ...
      lines.push(`    default: {`);
      lines.push(`      throw new Error('Expected message[${quote(discriminator.name)}] to be one of ' + JSON.stringify(${definition.name}) + ' ');`);
      lines.push(`    }`);
      lines.push(`  }`);
      lines.push("");
      lines.push("}");
    ...
    for (let j = 0; j < definition.fields.length; j++) {
      ...
      switch (fieldType) {
        ...
        default: {
          let type = definitions[fieldType];
          if (!type) {
            throw new Error("Invalid type " + quote(fieldType) + " for field " + quote(field.name));
          } else if (type.kind === "ENUM") {
    ...
    if (encoded === void 0) throw new Error('Expected ${quote(field.name + "Type")} to be one of ' + JSON.stringify(${type.name},null,2) + ' for enum ${quote(type.name)}');`;
  ...
  function compileSchemaJS(schema, isESM = false, withAllocator = false) {
    ...
    for (let i = 0; i < schema.definitions.length; i++) {
      ...
      if (isESM && definition.serializerPath?.length) {
        importsList.push(`import {encode${definition.name}, decode${definition.name}} from "${definition.serializerPath}";`);
      }
    }
    for (let i = 0; i < schema.definitions.length; i++) {
      let definition = schema.definitions[i];
      ...
      switch (definition.kind) {
        ...
        case "ENUM": {
          ...
          js.push("const " + definition.name + " = " + JSON.stringify(value, null, 2) + ";");
          js.push("const " + definition.name + "Keys = " + JSON.stringify(keys, null, 2) + ";");
          break;
        }
        case "UNION": {
          let value = {};
          ...
          js.push("const encode" + definition.name + "ByType = (function() { return [" + encoderName + "]; })()");
        }
        case "STRUCT":
        case "MESSAGE": {
          exportsList.push("decode" + definition.name, "encode" + definition.name);
          if (!isESM || !definition.serializerPath?.length) {
            js.push("");
            js.push(compileDecode("decode" + definition.name, definition, definitions, withAllocator, aliases));
            js.push("");
            js.push(compileEncode("encode" + definition.name, definition, definitions, aliases));
          }
          break;
        }
        default: {
